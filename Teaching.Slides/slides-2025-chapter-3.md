...
# **Aufgabe**: Simple HTML-Form Server (ASP.NET Core)
...
Kopieren Sie den Code und führen sie ihn aus:

```csharp {monaco} { lineNumbers: 'on', height: '300px' }
var builder = WebApplication.CreateBuilder(args);
var app = builder.Build();
...
# **Aufgabe**: Übermitteln von Dateien via HTML-Form

- Übermitteln von Dateien an den Webserver via HTML-Form
- Anzeigen der hochgeladenen Datei auf dem Client via HTML-Response

Kopieren Sie den Code und führen sie ihn aus:

```csharp {monaco} { lineNumbers: 'on', height: '330px' }
var builder = WebApplication.CreateBuilder(args);
var app = builder.Build();
